...
# 재료가 아니라 요리 이름을 받는다. 이 결과는 식단 조언을 물을 때 그대로
# 문장에 들어가는데, "비빔밥, 당근, 표고버섯, 고추장"처럼 재료가 섞이면
# 질문이 어색해지고 정작 무엇을 먹는지가 흐려진다.
FOOD_RECOGNITION_INSTRUCTION = (
    "사진에 있는 음식을 요리 단위로 알아내 정확한 한국어 명칭으로 나열해라. "
    "비빔밥의 당근·고추장처럼 한 요리에 들어간 재료는 따로 적지 말고 "
    "요리 이름 하나로 묶어라. 쌈장·간장 같은 양념장도 적지 마라. "
    "서로 다른 요리가 여러 개면 각각 적어라. "
